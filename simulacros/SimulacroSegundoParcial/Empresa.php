...
class Empresa
{
	...
	/**
	 * recorre la colección de ventas realizadas por la empresa y retorna una colección de ventas de motos importadas. Si en la venta al menos una de las motos es importada la venta debe ser informada.
	 *
	 * @return array
	 */
	public function informarVentasImportadas()
	{
		$colVentaMotosImportadas = [];
		foreach ($this->getColObjVenta() as $venta) {
			array_push($colVentaMotosImportadas,$venta->retornarMotosImportadas());
		}
		if (count($colVentaMotosImportadas)>0) {
			$this->setColObjVenta($colVentaMotosImportadas);
		}
		return $colVentaMotosImportadas;
	}
}//!Cierre clase